      ...
      success: function (response) {
        ...
        if ((args.hasOwnProperty('clean_date') && args.clean_date)) {
          for (var i = 0; i < response.items.length; i++) {
            if (response.items[i].start.dateTime) {
              response.items[i].end.dateTime = adjustDate(response.items[i].end.dateTime, true);
              response.items[i].start.dateTime = adjustDate(response.items[i].start.dateTime, true);
            } else if (response.items[i].start.date) {
              // whole day events have the date field while others have the dateTime field
